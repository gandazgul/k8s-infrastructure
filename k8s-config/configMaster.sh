...
MASTER_IP=`echo "${MASTER_SERVER}" | awk -F: '{print $2}' | awk -F/ '{print $3}'`
...
#kubectl patch service kubernetes-dashboard --namespace=kube-system --patch '{"spec": {"externalIPs": ["'${MASTER_IP}'"]}}'
DASHBOARD_USER_SECRET=`kubectl -n kube-system get secret | grep admin-user | awk '{print $1}'`
if [ ! -z "$DASHBOARD_USER_SECRET" ]; then
    echo "\nDashboard installed! visit: https://$MASTER_IP:8443/\n"
